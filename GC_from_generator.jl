...
# This program implements the (averaged) message passing algorithm and Monte Carlo simulation
# on random network. (Poisson network and Scale-free networks)

using Plots

function poissonGenerator(N::Int64, cm::Real)

    # Generatea a N-node Poisson Network with averaged degree cm.
    ...
    return k1, knn1, sigmaN, sigmaT
end

function scalefreeGenerator(N::Int64, gamma::Real)

    # Generate a N-node Power-law distributed network with parameter γ.

    norm = 0.0
    kx = zeros(Float64, N)
    cutoff = sqrt(Float64(N))  # Structural cutoff of uncorrelated ensemble
    for i = 1: N
        kx[i] = rand() ^ (-1. / (gamma - 1.0))
        while kx[i] > cutoff
            kx[i] = rand() ^ (-1. / (gamma - 1.0))
        end
        norm += kx[i]
...
function messagePassing(N::Int64, kc::Int64, k1::Vector{Int64}, knn1, rho::Float64,
                        sigmaT::Matrix{Float64}, sigmaN::Matrix{Float64}, err_tol::Float64)
...
function Recurrence(i::Int64, cluster_size::Int64, clusterID::Int64, clusterID_map::Vector{Int64},
                    k1::Vector{Int64}, knn1::Matrix{Int64}, 
                    y::Matrix{Int64})
...
function main()
    ...
    kc = 10
    rho = 0.
    k1, knn1, sigmaT, sigmaN = poissonGenerator(N, cm)
    # k1, knn1, sigmaT, sigmaN = scalefreeGenerator(N, gamma)
