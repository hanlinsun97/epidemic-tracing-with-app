...
// This program implements the Monte Carlo simulation on random Poisson networks.
...
#define cm 4.0              // Average degree of Poisson distribution
...
int *vis1,
*size_cluster1, **knn1, *k1, c1, c2, c3;
int **y, z;
...
{
    int i, j, n, it, ncluster1, cluster_size, m1, nrun, nc, np, nc2;
    int s1, s2, Nc1, Nc2, Nc3, N0, **s, nrun2, **denR, *T, in;
    double p, f, **x, **n1, **n2, MGC1, nsum1, nsumold1, nsum2, nsumold2, **GCA, rho, *Tx;
    int ausi, GC;
    double pc, kav, kappa, kappaT, Norm1, pc_sum, *pc_list, sum_degree;
    char string2[50];
    ...
    sprintf(string2, "MonteCarlo_Poi.txt");
    ...
    for (nrun = 0; nrun < Nrunmax; nrun++)
    {
        ...
        for (nc = 0; nc < 1; nc++) // Only care about rho=0 case. Calculate for all rho from 0 to 1 by setting nc = 100
        {
            ...
            // Average of uncorrelated network ensemble ends.
    ...
    for (nc = 0; nc < 1; nc++)  // Only care about rho=0. Calculate for all rho from 0 to 1 by setting nc = 100
    {
        rho = (double)nc * 0.01;
        for (nc2 = 0; nc2 < 101; nc2++)
        {
